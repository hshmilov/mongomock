...
class TestEnforcementSanity(TestBase):
    ...
    def test_no_scheduling(self):
        ...
        self.enforcements_page.wait_for_table_to_load()
        self.notification_page.verify_amount_of_notifications(1)
        wait_until(lambda: '1' in self.enforcements_page.get_column_data(self.FIELD_TIMES_TRIGGERED), interval=1)
    ...
    def test_enforcement_table_sort(self):
        ...
        self.enforcements_page.click_sort_column(self.FIELD_NAME)
        self.enforcements_page.wait_for_table_to_load()
        assert self.enforcements_page.get_column_data(self.FIELD_NAME) == list(reversed(enforcement_names))
        ...
        self.enforcements_page.click_sort_column(self.FIELD_QUERY_NAME)
        self.enforcements_page.wait_for_table_to_load()
        assert self.enforcements_page.get_column_data(self.FIELD_QUERY_NAME) == sorted(enforcement_queries)
        self.enforcements_page.click_sort_column(self.FIELD_QUERY_NAME)
        self.enforcements_page.wait_for_table_to_load()
        assert self.enforcements_page.get_column_data(self.FIELD_QUERY_NAME) == list(reversed(enforcement_queries))
