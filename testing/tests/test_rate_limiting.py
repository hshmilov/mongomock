...
def test_rate_limiting():
    ...
    for i in range(10):
        resp = gs.login_user(DEFAULT_USER, verify=False)
        assert resp.status_code == 200
        resp = gs.logout_user(verify=False)
        assert resp.status_code == 200

    # Set rate limiting to 5 per 3 minutes
    resp = gs.login_user(DEFAULT_USER, verify=False)
    assert resp.status_code == 200
    resp = gs.update_core_settings(NEW_SETTINGS)
    assert resp.status_code == 200
    ...
    resp = gs.login_user(DEFAULT_USER, verify=False)
    assert resp.status_code == 200
    resp = gs.update_core_settings(OLD_SETTINGS)
    assert resp.status_code == 200
