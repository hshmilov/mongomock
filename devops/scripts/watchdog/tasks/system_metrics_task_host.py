...
db_connection_line = 'mongodb://{user}:{password}@{addr}:{port}'.format(user='ax_user',
                                                                        password='ax_pass',
                                                                        addr='127.0.0.1',
                                                                        port=27017)
...
class SystemMetricsTask(WatchdogTask):
    ...
    def report_collection_stats(self):
        try:
            db_client = pymongo.MongoClient(db_connection_line)
            all_collections = sorted(list(db_client['aggregator'].list_collection_names()))

            for collection_name in all_collections:
                storage = get_collection_stats(db_client['aggregator'][collection_name])
    ...
    def report_unfinished_triggerables(self):
        try:
            db_client = pymongo.MongoClient(db_connection_line)
            for config in db_client['core']['configs'].find(
                    {
                        'status': 'up'
                    }
            ):
                pun = config['plugin_unique_name']
                for triggerable in db_client[pun]['triggerable_history'].find({
