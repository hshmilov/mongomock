...
ADAPTER_RESTART_COMMAND = f'sudo /sbin/runuser -l ubuntu -c "cd {CORTEX_PATH} && sudo ./axonius.sh system up --all ' \
                          '--prod --restart --env NODE_INIT_NAME={init_name}"'
STOP_SYSTEM_COMMAND = 'sudo /sbin/runuser -l ubuntu -c "cd /home/ubuntu/cortex && sudo ./axonius.sh system down --all"'
