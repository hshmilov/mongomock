...
INIT_FILE=/home/.axonius_done_init_host
if [ -e "$INIT_FILE" ]; then
...
_wait_for_apt install locales time
...
_wait_for_apt install -yq apt-transport-https ca-certificates curl software-properties-common makeself moreutils # required for https-repos
...
cd $SCRIPT_DIR
cp /tmp/nexus-apt /etc/apt/apt.conf.d/nexus
...
_wait_for_apt install -yq python3.6 python3.6-dev ipython python-pip htpdate unixodbc-dev
...
cp /tmp/weave-2.7.0 /usr/local/bin/weave
...
if [ $(cat /etc/passwd | grep netconfig | wc -l) -ne 0 ]; then
    ...
    cp /tmp/login.c /home/netconfig/login.c
    cp /tmp/login.py /home/netconfig/login.py
    cd /home/netconfig
...
set +e
cp /tmp/ZscalerRootCertificate-2048-SHA256.crt /usr/local/share/ca-certificates/
