...
def test_get_fsmo_roles(ldap_connection: LdapConnection):
    ...
    assert fsmo_dict['rid_master'] == 'gcp-dc-test.TestDomain.test'
    assert fsmo_dict['infra_master'] == 'gcp-dc-test.TestDomain.test'
    assert fsmo_dict['naming_master'] == 'gcp-dc-test.TestDomain.test'
...
def test_get_report_statistics(ldap_connection: LdapConnection):
    ...
    assert {'name': 'Naming Master', 'value': 'gcp-dc-test.TestDomain.test'} in fs['Forest Summary']
    assert {'name': 'Exchange Version', 'value': '2016'} in fs['Forest Features']
    assert {'name': 'Site Subnet Count', 'value': 9} in fs['Site Summary']
    assert {
