# pylint: disable=line-too-long,invalid-string-quote,redefined-builtin,too-many-locals,too-many-branches,too-many-statements,no-else-return,consider-using-enumerate,inconsistent-return-statements,unused-import
import base64
import struct
import binascii
import sys
    ...
    if lines[0].startswith('PuTTY-User-Key-File-2:'):
        ...
        if hexmac:
            ...
            if len(passphrase) > 0:
                ...
                # decrypt privbin
                aes256key = hashlib.sha1(b"\x00\x00\x00\x00" + passphrase.encode()).digest() + \
                    hashlib.sha1(b"\x00\x00\x00\x01" + passphrase.encode()).digest()
        ...
        if keytype == b'ssh-rsa':
            ...
            sequence = b"\x02\x01\x00" + b''.join(b"\x02\x82" + struct.pack('>H',
                                                                            len(s)) + s for s in [modulus, pubexp, privexp, p, q, e1b, e2b, iqmp])
            ...
            lastpart = struct.pack('>II', 1, 1) + pubopenssh + struct.pack('>I', len(prival) +
                                                                           len(pubval)) + prival + pubval + struct.pack('>I', len(comment)) + comment
            lastpart = lastpart + b"\x01\x02\x03\x04\x05\x06\x07"[0:8 - (len(lastpart) % 8)]
            complete = b"openssh-key-v1\x00\x00\x00\x00\x04none\x00\x00\x00\x04none" + \
                struct.pack('>III', 0, 1, len(pubopenssh)) + pubopenssh + struct.pack('>I', len(lastpart)) + lastpart
            completeb64 = base64.b64encode(complete).decode()
