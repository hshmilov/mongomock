...
class BitbucketConnection(RESTConnection):
    ...
    def _get_projects(self):
        yield from self._paged_get(PROJECTS_SUFFIX, device_type='projects')

    def _get_repos_by_project(self, project_name, max_commits=DEFAULT_MAX_COMMITS):
        repos_url = REPOS_SUFFIX.format(project_name=project_name)
        for repo in self._paged_get(repos_url, device_type='repos'):
    ...
    def _get_all_repos(self, max_commits=DEFAULT_MAX_COMMITS):
    ...
    def _paged_get(self, url_suffix: str, device_type: str, max_devices: int = MAX_NUMBER_OF_DEVICES):
        try:
