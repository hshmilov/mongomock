...
from oracle_vm_adapter.exceptions import OracleVmException
...
class OracleVmAdapter(AdapterBase):
    ...
    def _get_client_id(self, client_config):
        return client_config['OracleVm_Domain']

    def _connect_client(self, client_config):
        try:
            connection = OracleVmConnection(
                domain=client_config["OracleVm_Domain"], verify_ssl=client_config["verify_ssl"])
            connection.set_credentials(username=client_config["username"], password=client_config["password"])
            with connection:
                pass  # check that the connection credentials are valid
            return connection
        except OracleVmException as e:
            message = "Error connecting to client with domain {0}, reason: {1}".format(
                client_config['OracleVm_Domain'], str(e))
            logger.exception(message)
            raise ClientConnectionException(message)

    def _query_devices_by_client(self, client_name, client_data):
        """
        Get all devices from a specific OracleVm domain

        :param str client_name: The name of the client
        :param obj client_data: The data that represent a OracleVm connection

        :return: A json with all the attributes returned from the OracleVm Server
        """
        with client_data:
    ...
    def _parse_raw_data(self, devices_raw_data):
        servers = devices_raw_data['servers']
        vms = devices_raw_data['vms']
        for device_raw in servers:
            try:
                ...
                device.type = OracleVmDeviceType.OracleServer
                ...
                etherent_ports_data = device_raw.get("etherent_ports_data", [])
