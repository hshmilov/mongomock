...
class PuppetServerConnection:
    ...
    def __init__(self, logger, puppet_server_Address, puppet_username, puppet_password):
        ...
        :param str puppet_server_Address: Server adress (name of IP)
        ...
        self.base_puppet_url = config.PUPPET_CONNECTION_METHOD + self.puppet_server_Address + config.PUPPET_PORT_STRING       
        self._check_certificates_existence()

    def _make_certificates_directory_and_init_certificates_files_names(self):
        """ This function create directories for the SSL certificates of the connnection.
    ...
    def _enter_root_password_and_log_output_and_error(self, command):
        ...
        """
        try:
            ssh_stdin, ssh_stdout, ssh_stderr = self.ssh_client_connection.exec_command(command)
    ...
    def _generate_certificates_at_puppet_server(self):
        """ This function uses ssh to connect to the puppet server and creates the ssl certificates in the server.
         :raises a PuppetException if a problems occurs in the ssh connection
        """
        self.clean_certificates_command = config.CERTIFICATES_CLEAN_COMMAN + self.certificate_name_in_puppet_server
        self.generate_certificate_command = config.CERTIFICATES_CREATE_COMMAND + self.certificate_name_in_puppet_server
    ...
    def _copy_certificates_to_local_space(self):
        ...
        self.certificate_full_path_puppet_server = config.CERTIFICATE_LOCATION_IN_PUPPET_SERVER + \
            self.certificate_name_in_puppet_server + config.CERTIFICATES_SUFFIX
        try:
            self.scp_puppet_sever_connection = scp.SCPClient(self.ssh_client_connection.get_transport())
            # Copy CA certificate
    ...
    def _generate_certificates_and_copy_to_local_storage(self):
        ...
        try:
            ...
            self.ssh_client_connection.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ...
    def _check_certificates_existence(self):
        ...
        try:
            if self.ssh_lock.acquire(timeout=60*5) is True:  # Blocks for 5 minutes
                ...
                self.logger.error("Timout of ssh process")
                raise exceptions.PuppetException("Timout of ssh process")
        finally:
    ...
    def get_device_list(self):
        ...
        s = requests.Session()
        s.cert = (self.local_certificate_file_path, self.local_private_key_file_path)
        ...
        for json_node in parsed_query_nodes_json:
            try:
                ...
                self.logger.error("Error in getting information about node:{0}. Error:{1}".format(str(json_node),
                                                                                                  str(err)))
    ...
    def _query_fact_device(self, opened_session, basic_json_node=None):
        """ This function gets a json with basic information about a node (or nothing if not needed),
            ...
            :param requests.Session opened_session: An opened session (to increase performence)
            :param dict basic_json_node A dictinary with basic puppet information about the device
            :raises a PuppetException if a problems occurs in the response from the server or if ssh problems.
        ...
        facts_query_suffix = config.PUPPET_API_PREFIX + "/nodes/" + basic_json_node[u'certname'] + "/facts"
        ...
        for json_fact in parsed_query_facts:
            # Add check in dictinary
            device[json_fact[u'name']] = json_fact[u'value']
        return device
