...
from logging import _nameToLevel, getLevelName, getLogger
try:
...
class SearchCommand(object):
    ...
    def logging_level(self, value):
        ...
        if isinstance(value, (bytes, six.text_type)):
            try:
                level = _nameToLevel[value.upper()]
            except KeyError:
    ...
    def search_results_info(self):
        ...
        info = ObjectView(dict(map(lambda f, v: (convert_field(f), convert_value(v)), zip(fields, values))))
        ...
        try:
            ...
            messages = filter(lambda t, m: t or m, zip(msg_type.split('\n'), msg_text.split('\n')))
    ...
    _metadata_map = {
        'action':
            (lambda v: 'getinfo' if v == '__GETINFO__' else 'execute' if v ==
             '__EXECUTE__' else None, lambda s: s.argv[1]),
        'preview':
    ...
    class ConfigurationSettings(object):
        ...
        def __str__(self):
            ...
            """
            text = ', '.join(map(lambda name, value: name + '=' +
                                 json_encode_string(six.text_type(value)), six.iteritems(self)))
            return text
        ...
        def iteritems(self):
            ...
            return filter(
                lambda name, value: value is not None, map(
