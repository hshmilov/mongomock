...
from carbonblack_defense_adapter.exceptions import CarbonblackDefenseAlreadyConnected, CarbonblackDefenseConnectionError,\
    CarbonblackDefenseNotConnected, \
    CarbonblackDefenseRequestException


class CarbonblackDefenseConnection(object):
    def __init__(self, domain, verify_ssl):
        """ Initializes a connection to CarbonblackDefence using its rest API

        :param str domain: domain address for CarbonblackDefence
        """
        self.domain = domain
        url = domain
    ...
    def connect(self):
        ...
        if self.is_connected:
            raise CarbonblackDefenseAlreadyConnected()
        ...
        if self.connector_id is not None and self.apikey is not None:
            ...
            try:
                ...
                raise CarbonblackDefenseConnectionError(str(e))
        else:
            raise CarbonblackDefenseConnectionError("No user name or password")
    ...
    def _get(self, name, params=None):
        """ Serves a GET request to CarbonblackDefense API
        ...
        if not self.is_connected:
            raise CarbonblacDefensekNotConnected()
        params = params or {}
        ...
        try:
            ...
            raise CarbonblackDefenseRequestException(str(e))
        return response.json()

    def get_device_list(self, **kwargs):
        """ Returns a list of all agents

        :param dict kwargs: api query *string* parameters (ses CarbonblackDefense's API documentation for more info)
        :return: the response
        :rtype: dict
        """
        devices_list = []
