...
from tanium_adapter.consts import CACHE_EXPIRATION, CACHE_DT_FMT, PAGE_SIZE, MAX_DEVICES_COUNT, HEADERS
...
class TaniumPlatformConnection(tanium.connection.TaniumConnection):
    ...
    def advanced_connect(self, client_config):
        self._tanium_get('system_status', options={'row_count': 1, 'row_start': 0})
    ...
    def get_device_list(self, client_name, client_config):
        ...
        last_reg_mins = client_config.get('last_reg_mins', 60)
        ...
        if last_reg_mins:
            last_reg_dt = self._cache_filter_dt(tanium.tools.dt_ago_mins(mins=last_reg_mins))
        ...
        while row_start < MAX_DEVICES_COUNT:
            try:
                ...
                objs = self._tanium_get('system_status', options=options)
