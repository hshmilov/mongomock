...
def _encode_multipart_formdata(fields):
    ...
    BOUNDARY = '--' + email.generator.choose_boundary()
...
class _CommandInvoker:
    ...
    def invoke(self, command, args={}):
        ...
        if output != self.output:
            log(logging.INFO,
                "Invocation of '%s' requests '%s' output rather than default '%s'" % (command, output, self.output))
    ...
    def get_response(self, url, fileargs={}):
        try:
            ...
            log_and_raise_error(logging.ERROR,
                                'The server %s:%s could not fulfill the request. %s' % (self.host, self.port, str(e)))
        except URLError as e:
            log_and_raise_error(logging.ERROR,
                                'Failed to reach the server %s:%s. Error/reason: %s' % (self.host, self.port, str(e)))
    ...
    def parse_response(self, response):
        ...
        if status == 'OK':
            ...
            log_and_raise_error(logging.ERROR,
                                'Unknown error occurred.  Status: (%s) Result: %s' % (d["status"], d["result"]))
        return d['result']
    ...
    def build_url_request(self, command, args):
        ...
        """
        query_string = "&".join(
            ["%s=%s" % (urllib.parse.quote_plus(str(k)), urllib.parse.quote_plus(v)) for k, v in list(args.items())])
