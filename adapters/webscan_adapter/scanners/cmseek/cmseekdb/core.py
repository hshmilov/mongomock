...
import webscan_adapter.scanners.cmseek.VersionDetect.detect as version_detect  # Version detection
import webscan_adapter.scanners.cmseek.deepscans.core as advanced  # Deep scan and Version Detection functions
import webscan_adapter.scanners.cmseek.cmseekdb.basic as cmseek  # All the basic functions
import webscan_adapter.scanners.cmseek.cmseekdb.sc as source  # Contains function to detect cms from source code
# Contains function to detect CMS from gathered http headers
import webscan_adapter.scanners.cmseek.cmseekdb.header as header
import webscan_adapter.scanners.cmseek.cmseekdb.cmss as cmsdb  # Contains basic info about the CMSs
import webscan_adapter.scanners.cmseek.cmseekdb.robots as robots
import webscan_adapter.scanners.cmseek.cmseekdb.generator as generator
import webscan_adapter.scanners.cmseek.cmseekdb.result as result
...
def main_proc(site, cua):
        ...
        if site != init_source[3] and site + '/' != init_source[3]:
            if cmseek.redirect_conf == '0':
                ...
                follow_redir = input('[#] Set ' + cmseek.bold + cmseek.fgreen +
                                     init_source[3] + cmseek.cln + ' as target? (y/n): ')
                ...
                site = init_source[3]
                cmseek.info("Followed redirect, New target: " + cmseek.bold +
    ...
    if cms_detected == '1':
        cmseek.success('CMS Detected, CMS ID: ' + cmseek.bold + cmseek.fgreen + cms + cmseek.cln +
                       ', Detection method: ' + cmseek.bold + cmseek.lblue + detection_method + cmseek.cln)
