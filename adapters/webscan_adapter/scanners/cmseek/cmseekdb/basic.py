...
import time
import re
from webscan_adapter.scanners.cmseek.cmseekdb.getsource import *
from webscan_adapter.scanners.cmseek.cmseekdb.config import *
...
""".format(orange, lblue, yellow, cmseek_version, red, white))
    if txt != "":
        ...
        print(cln + bold + lbluebg + black + " Author: " + cln + bold + " https://twitter.com/r3dhax0r" +
...
def bye():
    bye_dict = ["adios", "adieu", "addio", "adeus", "aloha", "arrivederci", "auf Wiedersehen", "au revoir", "sayonara",
                "shalom", "totsiens", "vale", "zaijian", "Aabar dekha hobey", "Fir milenge", "Annyeong", "Ja mata ne", "До Встречи"]
...
def update():
    ...
    get_version = getsource(
        'https://raw.githubusercontent.com/Tuhinshubhra/CMSeeK/master/current_version', randomua('generate'))
    if get_version[0] != '1':
        ...
        if my_version > serv_version:
            ...
            error("Either you or me (The Developer) messed things up.\n" + cln +
                  "[↓] Download the proper version from: " + fgreen + bold + GIT_URL)
            ...
            if update_me.lower() == 'y':
                ...
                try:
                    ...
                    if os.path.isfile(lock_file):
                        ...
                        os.remove(lock_file)
                    subprocess.run(("git checkout . && git pull %s HEAD") %
                                   GIT_URL, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def wpbrutesrc(url, user, pwd):
    ...
    try:
        ckreq = urllib.request.Request(
            url,
            data=urllib.parse.urlencode({'log': user, 'pwd': pwd, 'wp-submit': 'Log In',
                                         'redirect_to': redirecto}).encode("utf-8"),
            headers={
                'User-Agent': ua
            }
        )
