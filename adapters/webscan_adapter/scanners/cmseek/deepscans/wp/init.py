...
import webscan_adapter.scanners.cmseek.cmseekdb.basic as cmseek  # Good old module
import webscan_adapter.scanners.cmseek.VersionDetect.wp as wordpress_version_detect
import webscan_adapter.scanners.cmseek.deepscans.wp.userenum as wp_user_enum
import webscan_adapter.scanners.cmseek.deepscans.wp.vuln as wp_vuln_scan
import webscan_adapter.scanners.cmseek.deepscans.wp.pluginsdetect as wp_plugins_enum
import webscan_adapter.scanners.cmseek.deepscans.wp.themedetect as wp_theme_enum
import webscan_adapter.scanners.cmseek.deepscans.wp.pathdisc as path_disclosure
import webscan_adapter.scanners.cmseek.deepscans.wp.check_reg as check_reg
import webscan_adapter.scanners.cmseek.cmseekdb.result as sresult
import time
import re
    ...
    if id == "wp":
        ...
        if detection_method == 'source':
            ...
            if not re.search(wp_match_pattern, source):
                ...
                cmseek.success('Run CMSeeK with {0}{1}{2} argument next time'.format(
        ...
        if licfile == '1':
            cmseek.update_log('license_file', url + '/license.txt')
        ...
        if wpupdir == '1':
            ...
            if item_initiated == False:
                sresult.init_item("Uploads directory has listing enabled: " +
                                  cmseek.fgreen + url + '/wp-content/uploads' + cmseek.cln)
            else:
        ...
        if plugins_found != 0:
            ...
            for i, plugin in enumerate(plugins):
                ...
                if i == 0 and i != plugs_count - 1:
                    ...
                    sresult.end_subsub('URL: ' + cmseek.fgreen + url + '/wp-content/plugins/' +
                                       plug[0] + cmseek.cln, True, False)
                else:
                    sresult.empty_sub()
        ...
        if themes_found != 0:
            ...
            for i, theme in enumerate(themes):
                ...
                if i == 0 and i != thms_count - 1:
                    ...
                    if thmz[1] != '':
                        sresult.subsub('Theme Zip: ' + cmseek.bold + cmseek.fgreen +
                                       url + thmz[1] + cmseek.cln, True, False)
                    sresult.end_subsub('URL: ' + cmseek.fgreen + url + '/wp-content/themes/' +
        ...
        if version != '0':
            ...
            cmseek.update_log('cms_version', version)
            if wpvdbres == '1':
                ...
                if vulnss > 0:
                    for i, vuln in enumerate(result['vulnerabilities']):
                        if i == 0 and i != vulnss - 1:
                            ...
                            sresult.init_subsub("Type: " + cmseek.bold + cmseek.fgreen +
                                                str(vuln['vuln_type']) + cmseek.cln, False, True)
                            sresult.subsub("Link: " + cmseek.bold + cmseek.fgreen +
                                           "http://wpvulndb.com/vulnerabilities/" + str(vuln['id']) + cmseek.cln, False, True)
                            strvuln = str(vuln)
                            if 'cve' in strvuln:
                                for ref in vuln['references']['cve']:
                                    sresult.subsub(
                                        "CVE: " + cmseek.fgreen + "http://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-" + str(ref) + cmseek.cln, False, True)

                            if 'exploitdb' in strvuln:
                                for ref in vuln['references']['exploitdb']:
                                    sresult.subsub("ExploitDB Link: " + cmseek.fgreen +
                                                   "http://www.exploit-db.com/exploits/" + str(ref) + cmseek.cln, False, True)

                            if 'metasploit' in strvuln:
                                for ref in vuln['references']['metasploit']:
                                    sresult.subsub("Metasploit Module: " + cmseek.fgreen +
                                                   "http://www.metasploit.com/modules/" + str(ref) + cmseek.cln, False, True)

                            if 'osvdb' in strvuln:
                                for ref in vuln['references']['osvdb']:
                                    sresult.subsub("OSVDB Link: " + cmseek.fgreen +
                                                   "http://osvdb.org/" + str(ref) + cmseek.cln, False, True)

                            if 'secunia' in strvuln:
                                for ref in vuln['references']['secunia']:
                                    sresult.subsub("Secunia Advisory: " + cmseek.fgreen +
                                                   "http://secunia.com/advisories/" + str(ref) + cmseek.cln, False, True)

                            if 'url' in strvuln:
                                for ref in vuln['references']['url']:
                                    sresult.subsub("Reference: " + cmseek.fgreen + str(ref) + cmseek.cln, False, True)

                            sresult.end_subsub("Fixed In Version: " + cmseek.bold + cmseek.fgreen +
                                               str(vuln['fixed_in']) + cmseek.cln, False, True)

                        elif i == vulnss - 1:
                            sresult.empty_sub(False)
                            sresult.end_sub(cmseek.bold + cmseek.fgreen + str(vuln['title']) + cmseek.cln, False)
                            sresult.init_subsub("Type: " + cmseek.bold + cmseek.fgreen +
                                                str(vuln['vuln_type']) + cmseek.cln, False, False)
                            sresult.subsub("Link: " + cmseek.bold + cmseek.fgreen +
                                           "http://wpvulndb.com/vulnerabilities/" + str(vuln['id']) + cmseek.cln, False, False)
                            strvuln = str(vuln)
                            if 'cve' in strvuln:
                                for ref in vuln['references']['cve']:
                                    sresult.subsub(
                                        "CVE: " + cmseek.fgreen + "http://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-" + str(ref) + cmseek.cln, False, False)

                            if 'exploitdb' in strvuln:
                                for ref in vuln['references']['exploitdb']:
                                    sresult.subsub("ExploitDB Link: " + cmseek.fgreen +
                                                   "http://www.exploit-db.com/exploits/" + str(ref) + cmseek.cln, False, False)

                            if 'metasploit' in strvuln:
                                for ref in vuln['references']['metasploit']:
                                    sresult.subsub("Metasploit Module: " + cmseek.fgreen +
                                                   "http://www.metasploit.com/modules/" + str(ref) + cmseek.cln, False, False)

                            if 'osvdb' in strvuln:
                                for ref in vuln['references']['osvdb']:
                                    sresult.subsub("OSVDB Link: " + cmseek.fgreen +
                                                   "http://osvdb.org/" + str(ref) + cmseek.cln, False, False)

                            if 'secunia' in strvuln:
                                for ref in vuln['references']['secunia']:
                                    sresult.subsub("Secunia Advisory: " + cmseek.fgreen +
                                                   "http://secunia.com/advisories/" + str(ref) + cmseek.cln, False, False)
                            ...
                            sresult.end_subsub("Fixed In Version: " + cmseek.bold + cmseek.fgreen +
                                               str(vuln['fixed_in']) + cmseek.cln, False, False)
                        else:
                            sresult.empty_sub(False)
