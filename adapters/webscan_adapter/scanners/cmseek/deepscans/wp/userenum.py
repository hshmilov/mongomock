...
def start(id, url, ua, ga, source):
    ...
    jpapisrc = cmseek.getsource('https://public-api.wordpress.com/rest/v1.1/sites/' +
                                strippedurl + '/posts?number=100&pretty=true&fields=author', ua)
    if jpapisrc[0] != '1' or 'login' not in jpapisrc[1]:
        ...
        for user in json.loads(jpapisrc[1])['posts']:
            if user['author']['login'] not in str(jpapiuser):
                ...
                cmseek.success("Found user from Jetpack api : " + cmseek.fgreen +
                               cmseek.bold + user['author']['login'] + cmseek.cln)
    ...
    if len(usernames) > 0:
        ...
        if len(usernames) == 1:
            cmseek.success(cmseek.bold + cmseek.fgreen + str(len(usernames)) +
                           " Usernames" + " was enumerated" + cmseek.cln)
        else:
            cmseek.success(cmseek.bold + cmseek.fgreen + str(len(usernames)) +
                           " Usernames" + " were enumerated" + cmseek.cln)
    else:
