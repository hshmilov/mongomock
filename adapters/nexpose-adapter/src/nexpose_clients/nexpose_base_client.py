...
class NexposeClient(abc.ABC):
    ...
    def __init__(self, logger, num_of_simultaneous_devices, host, port, username, password, verify_ssl):
        ...
        if not self._does_api_exist():
            raise axonius.adapter_exceptions.ClientConnectionException("API Does not Exist.")
        super().__init__()
    ...
    def parse_raw_device_last_seen(last_seen):
        try:
            if last_seen is None:
                ...
                raise axonius.adapter_exceptions.ParseDevicesError(
                    "An Exception was raised while getting and parsing the last_seen field.")
