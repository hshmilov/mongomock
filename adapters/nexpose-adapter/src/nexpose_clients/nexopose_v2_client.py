...
class NexposeV2Client(nexpose_base_client.NexposeClient):

    @contextmanager
    def _get_session(self):
        """A context manager for session object, to be used only with."""
        @retry(stop_max_attempt_number=10, wait_fixed=5000, retry_on_result=retry_if_result_false)
        ...
        session = nexpose.NexposeSession.Create(self.host, self.port, self.username, self.password)
        connected = _wait_for_status(session, nexpose.NexposeStatus.NORMAL_MODE)
        if not connected:
            raise axonius.adapter_exceptions.ClientConnectionException(
                "Did not successfully create session with nexpose client.")
        session.Open()
        try:
            yield session
        except Exception as err:
            raise axonius.adapter_exceptions.AdapterException('An exception occurred while in session')
        finally:
            session.Close()
    ...
    def get_all_devices(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_of_simultaneous_devices) as executor:
            try:
    ...
    def _does_api_exist(self):
        with self._get_session() as session:
            successfully_connected = isinstance(session, nexpose.NexposeSession)
