    ...
    def get_device_list(self):
        ...
        for agent_raw in agents:
            ...
            try:
                ...
                logger.warning(f'Problem with open_ports for agent_raw {agent_raw}', exc_info=True)

            try:
                agent_raw['advanced_config'] = self._get(
                    f'conf/agents/{agent_name}/Settings/AdvancedConfiguration')['agent-config']
            except Exception:
                logger.warning(f'Problem with advanced_config for agent_raw {agent_raw}', exc_info=True)

            try:
                agent_raw['general_details'] = self._get(f'conf/agents/{agent_name}/GeneralDetails')
            except Exception:
                logger.warning(f'Problem with general_details for agent_raw {agent_raw}', exc_info=True)

            try:
                agent_raw['cpu_usage'] = self._get(f'conf/{agent_name}/Settings/CPUUsageRestraining')
            except Exception:
                logger.warning(f'Problem with cpu_usage for agent_raw {agent_raw}', exc_info=True)

            try:
                agent_raw['discovery_settings'] = self._get(f'conf/agents/{agent_name}/Settings/DiscoverySettings')
            except Exception:
                logger.warning(f'Problem with DiscoverySettings for agent_raw {agent_raw}', exc_info=True)

            try:
                agent_raw['agent_tags'] = self._get(f'conf/agents/{agent_name}/tags/')['tags']
            except Exception:
                logger.warning(f'Problem with agent_tags for agent_raw {agent_raw}', exc_info=True)
