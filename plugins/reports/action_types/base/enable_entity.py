...
class EnableEntities(ActionTypeBase):
    """
    Uses enabelable to disable entities
    """
    ...
    def _run(self) -> EntitiesResult:
        feature_name, urlpath = {
            EntityType.Devices: ('Devicedisabelable', 'devices/enable'),
            EntityType.Users: ('Userdisabelable', 'users/enable')
        }[self._entity_type]
        ...
        for plugin_unique_name in relevant_plugin_unique_names:
            for chunk in chunks(10000, self._internal_axon_ids):
                ...
                if response.status_code != 200:
                    logger.error(f'Error on enabling on {plugin_unique_name}: {response.content}')
                    yield from (EntityResult(id_, False, response.content) for id_ in chunk)
                else:
                    yield from (EntityResult(id_, True, 'Enabled entity') for id_ in chunk)
