...
def endpoint_action(adapter_name, action_name, current_result, node_id, tags_dict=None):
        ...
        try:
            for adapter_data in entry['adapters']:
                if adapter_data['plugin_name'] == adapter_name:
                    pylum_id = adapter_data['data'].get('pylum_id')
                    machine_name = adapter_data['data'].get('name')
                    sid = adapter_data['data'].get('sid')
                    client_id = adapter_data['client_used']
                    response_dict = dict()
                    response_dict['pylum_id'] = pylum_id
                    response_dict['machine_name'] = machine_name
                    response_dict['sid'] = sid
                    response_dict['tags_dict'] = tags_dict
                    response_dict['client_id'] = client_id
