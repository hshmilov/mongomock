...
def get_ips_from_view_as_iter(current_result,
                              fetch_public_ips,
                              fetch_private_ips,
                              exclude_ipv6=False,
                        ...
                        if isinstance(nic.get('ips'), list):
                            for ip in nic.get('ips'):
                                ...
                                if fetch_private_ips and is_private:
                                    ...
                                    ips.add(ip)
            yield (ips, EntityResult(entry['internal_axon_id'], True, 'success'))
        except Exception:
            logger.exception(f'Failed adding nic entry {entry}')
            yield ([], EntityResult(entry['internal_axon_id'], False, 'Unexpected Error'))
