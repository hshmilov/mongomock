...
class CoreService(PluginBase, Configurable):
    ...
    def _generate_unique_name(self, plugin_name):
        ...
        for unique_name_suffix in [f"_{i}" for i in range(MAX_INSTANCES_OF_SAME_PLUGIN)]:
            ...
            if not self._get_config(plugin_unique_name=plugin_unique_name) and plugin_unique_name not in self.online_plugins:
                break
        else:
    ...
    def proxy(self, full_url):
        ...
        # If our request comes back with a content-encoding and it will be returned
        # in Response(..., header=headers) than it will override the content-encoding
        # generated by nginx
