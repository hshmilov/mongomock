...
def requires_aggregator():
    ...
    def wrap(func):
        def actual_wrapper(self, *args, **kwargs):
            if self._aggregator_unique_plugin_name is None:
...
class GUIPlugin(PluginBase):
    def __init__(self, *args, **kwargs):
        ...
        if aggregator is None:
            self._aggregator_unique_plugin_name = None
        else:
            self._aggregator_unique_plugin_name = aggregator['unique_plugin_name']
    ...
    def _query_aggregator(self, resource, *args, **kwargs):
        ...
        :return:
        """
        return self.request_remote_plugin(resource, self._aggregator_unique_plugin_name, *args, **kwargs).json()
    ...
    def adapter_device_by_id(self, device_id):
        ...
        with self._get_db_connection(False) as db_connection:
            parsed_db = db_connection[self._aggregator_unique_plugin_name]['parsed']
    ...
    def adapter_devices(self, limit, skip, fields):
        """
    ...
    @requires_aggregator()
    @paginated()
    @add_rule_unauthenticated("api/current_devices")
    def current_devices(self, limit, skip):
        """
        Get Axonius devices from the aggregator
        """
        with self._get_db_connection(False) as db_connection:
            client_collection = db_connection[self._aggregator_unique_plugin_name]['devices_db']
            return jsonify(
    ...
    def _get_plugin_schemas(self, db_connection, unique_plugin_name):
        ...
        clients_value = db_connection[unique_plugin_name]['adapter_schema'].find_one(sort=[('adapter_version',
                                                                                            pymongo.DESCENDING)])
