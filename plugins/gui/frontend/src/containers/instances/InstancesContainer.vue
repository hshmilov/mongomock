<template>
    <x-page title="instances" class="x-instances">
        <div align="right">
            <button id="get-connection-string" class="x-btn" @click="connecting= !connecting" :class="{disabled:isReadOnly}">Connect Node</button>
        </div>
        <x-table-actions title="Edit, Tag or Remove Instances" :loading="loading">
            ...
            <x-table v-if="instances" slot="table" id-field="node_id" :data="instances" :fields="fields"
                     v-model="isReadOnly ? undefined : selectedInstance" :click-row-handler="isReadOnly ? undefined : showNameChangeModal"/>
        </x-table-actions>
        <modal v-if="renaming" @close="closeNameChange" @confirm="instanceNameChange" approve-text="Change Name">
        ...
        <modal v-if="connecting && !isReadOnly" @close="connecting= !connecting" @confirm="connecting= !connecting" >
            <div slot="body">How to connect a new node<br/><br/>
                ...
                3. Paste a connection string that looks like (with spaces): {{ hostIP }} {{ connectionEncriptionKey }} &lt;User-Nickname&gt;
            </div>
            <div slot="footer">
        ...
        <modal id="delete-entities" v-if="deleting" @close="closeConfirmDelete" @confirm="doRemoveInstance" approve-text="Delete">
...
<script>
    ...
    export default {
        ...
        components: { xTable, Modal, xPage, xTableActions },
        computed: {
            ...mapState({
                isReadOnly(state) {
