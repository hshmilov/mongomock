<template>
    <x-card :title="`${data.title} Coverage`" class="x-coverage-card">
        <x-pie :data="pieSlices" @click-one="$emit('click-one', $event)" :id="data.name" :force-text="true"
               :read-only="readOnly"/>
    </x-card>
</template>
...
<script>
    ...
    export default {
        name: 'x-coverage-card',
        components: {xCard, xPie},
