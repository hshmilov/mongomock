            ...
            <div v-for="dose, index in data.instance_details" class="dose-item">
                ...
                <div @click="selectData(index)" class="dose-container" :class="{selected: selectedData === index}">
                    ...
                    <div class="dose-summary" v-if="dose.instance_state === 'Taken'">
                        <div>{{dose.vi}}mL</div>
                        <div class="dose-time">{{formatShortTime(dose.start_ts)}} - {{formatShortTime(dose.end_ts)}}
                        </div>
                    </div>
                </div>
            </div>
            ...
            detailsSchema() {
                return {
                    type: 'array', items: [
                        ...
                        {
                            type: 'number',
                            title: 'Percentage Infused',
                            name: 'percentage_of_volume_infused',
                            format: 'percentage'
                        }
                    ]
                }
