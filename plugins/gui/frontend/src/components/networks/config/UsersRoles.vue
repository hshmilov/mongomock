<template>
    <div class="x-users-roles">
            ...
            <div class="user-details">
                ...
                <div>
                    <div class="user-details-title">{{ user.user_name }} <span
                            v-if="user.first_name.length > 0 || user.last_name.length > 0"> - {{ user.first_name}} {{ user.last_name }}</span>
                    </div>
                    <div>{{ user.source }}</div>
                </div>
            </div>
        ...
        <x-modal v-if="userToCreate" @close="closeCreateUser" @confirm="performCreateUser" approve-text="Create User">
            <div slot="body">
                <x-form :schema="userSchema" v-model="userToCreate"/>
            </div>
        </x-modal>
        ...
        <x-modal v-if="rolesConfig.data" title="Manage Roles" class="roles-container" @close="closeRoles">
            <div slot="body">
                <div class="roles-default">
                    ...
                    <x-select :options="roleOptionsCustom" :searchable="true" :value="defaultRole"
