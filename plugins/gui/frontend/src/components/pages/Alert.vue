<template>
    <x-page :breadcrumbs="[
    	...
    	{ title: (alertData.name? alertData.name : 'New alerts')}
    ]">
        <div class="v-spinner-bg" v-if="loading"></div>
        <pulse-loader :loading="loading" color="#FF7D46"/>
        <x-box class="x-alert">
            <form>
                <!-- Section for alerts name and query to run by -->
                <div class="x-grid">
                    ...
                    <x-select :options="currentQueryOptions" v-model="currentQuery" placeholder="query..."
                              :searchable="true" id="alert_query" @input="tour('alertIncreased')" :align-agile="false"/>
                </div>

                <!-- Section for defining the condition which match of will trigger the alerts -->
                ...
                <div class="configuration">
                    ...
                    <div class="content">
                        ...
                        <x-checkbox class="grid-span2" label="New entities were added to query results"
                                    v-model="alert.triggers.new_entities"
                                    title="Trigger alert if the given saved query results contains new entities"/>
                        <x-checkbox class="grid-span2" label="Previous entities were subtracted from query results"
                                    v-model="alert.triggers.previous_entities"
                                    title="Trigger alert if entities were subtracted from the given saved query results"/>
                        <x-checkbox label="The number of query results is above" v-model="alert.triggers.increase"
                                    id="alert_increased"
                        ...
                        <div class="form-inline">
                            <input id="alert_above" type="number" v-model="alert.triggers.above" min="0"
                                   @keypress="validateInteger" @input="tour('alertAction')"
                                   :disabled="!alert.triggers.increase">
                        </div>
                ...
                <div class="configuration">
                    ...
                    <div class="content">
                        ...
                        <div></div>
                        <x-checkbox class="grid-span2" label="Create ServiceNow incident"
                                    v-model="actions.servicenowIncident"/>
                        <x-checkbox class="grid-span2" label="Create ServiceNow computer"
                                    v-model="actions.servicenowComputer"/>
                        <x-checkbox class="grid-span2" label="Create Jira ticket" v-model="actions.jiraTicket"/>
                        <x-checkbox class="grid-span2" label="Create FreshService incident"
                                    v-model="actions.freshserviceIncident"/>
                        ...
                        <x-checkbox class="grid-span2" label="Unisolate CB response devices"
                                    v-model="actions.cbUnisolate"/>
                        <!-- SYSLOG -->
                        <x-checkbox label="Notify syslog" v-model="actions.syslog" @change="checkSyslogSettings"/>
                        <x-checkbox v-if="actions.syslog" label="Send Device Data To Syslog"
                                    v-model="sendAllDevicesToSyslog"/>
                        <div v-if="!actions.syslog"></div>
                        <!-- SYSLOG -->
                        <!-- MAIL -->
                        <x-checkbox class="grid-span2" label="Send an Email" v-model="actions.mail"
                                    @change="checkMailSettings"/>
                        <div class="mail-config" v-if="actions.mail">
                            ...
                            <div class="x-grid">
                                ...
                                <x-checkbox label="Changes In Query Results In a CSV"
                                            v-model="sendDevicesChangesCSVToEmail"/>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        name: 'x-alert',
        components: {
            xSelect, xPage, xBox, xCheckbox, xModal, PulseLoader
        },
        computed: {
            ...mapState({
                ...
                currentQueryOptions(state) {
                    ...
                    return queries.map(q => {
                        return {...q, title: q.title ? q.title : q.name}
                    })
                },
            ...
            complete() {
                if (!this.alert.name) {
                    this.error = 'Please specify alerts name.'
                    return false
                }
        ...
        data() {
            return {
                ...
                actions: {
                    ...
                    syslog: false,
                    jiraTicket: false,
                    servicenowIncident: false,
                    servicenowComputer: false,
                    cbIsolate: false,
                    cbUnisolate: false,
                    freshserviceIncident: false
                },
                mailList: [],
                mailListCC: [],
                subject: '',
                ...
                if (this.actions.mail) {
                    this.alert.actions.push({
                        type: 'send_emails', data: {
                            ...
                            emailListCC: this.mailListCC,
                            sendDeviceCSV: this.sendDevicesCSVToEmail,
                            sendDevicesChangesCSV: this.sendDevicesChangesCSVToEmail
                        }
                    })
                }
                if (this.actions.tag) {
        ...
        created() {
            ...
            /* Fetch all saved queries for offering user to base alerts upon */
