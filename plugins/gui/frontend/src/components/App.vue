...
<script>
	...
	export default {
        name: 'app',
        components: {
            xLogin, xTopBar, xSideBar, xAccessModal, xToast, XGettingStarted, XFabTransition
        },
        data() {
            return {
                blockedComponent: '',
        ...
        computed: {
            ...mapState({
                gettingStartedAutoOpen: state => {
                    return _get(state, 'onboarding.gettingStarted.data.settings.autoOpen', false)
                },
        ...
        async mounted() {
            GettingStartedPubSub.$on('getting-started-open-state', this.changeChecklistOpenState)
            this.handleExpiration()
            this.getUser()
