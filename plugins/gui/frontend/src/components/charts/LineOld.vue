<template>
    <div class="x-line">
        <div v-if="config" class="subtitle">Results between {{ parseDate(config.datefrom) }} and {{ parseDate(config.dateto)}}:</div>
            ...
            <template v-for="(line, lineInd) in processedData">
                ...
                <g class="line-plot" :class="{ hover: lineInd === hoverLine, [ line.class ]: true }">
                    ...
                    <template v-for="segment in line.segments" v-if="segment.x1 && segment.y1">
                        <line :x1="segment.x1" :x2="segment.x2" :y1="segment.y1" :y2="segment.y2" class="plot-segment"
                              @mouseover="mouseoverLine($event, lineInd)" @mouseout="mouseoutLine"></line>
                    </template>
                    <template v-for="(segment, segmentInd) in line.segments">
                        <circle :cx="segment.x2" :cy="segment.y2" r="2" class="plot-point"
        ...
        <div v-show="showTooltip" class="tooltip" ref="tooltip">
            ...
            <div v-if="showPoint" class="tooltip-content">
                ...
                <div class="content-title">{{ processedData[hoverLine].segments[hoverPoint].title }}</div>
                <!-- Value for the title is the vertical quantity -->
                <div>{{ processedData[hoverLine].segments[hoverPoint].value }}</div>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        name: 'x-line',
        props: { data: {required: true}, config: {}},
        computed: {
            viewWidth() {
                return 240
            },
            ...
            processedData() {
                return this.data.lines.map((line, lineIndex) => {
                    ...
                    this.data.scale.forEach((scale, index) => {
                        ...
                        segments.push({
                            ...
                            title: scale, value: pointValue
                        })
                        prev = next
                    })
                    let colourIndex = (lineIndex + 1) % 6 || 6
                    return {
            ...
            horizontalSpace() {
                return (this.viewWidth - this.scaleOffset) / this.data.scale.length
            },
            verticalScale() {
                // Find highest result
                let maxResult = -1
                this.data.lines.forEach((line) => {
                    Object.values(line.points).forEach((value) => {
                        if (value > maxResult) {
                            maxResult = value
                        }
                    })
            ...
            viewScalePortion() {
                return (this.viewHeight - this.verticalSpace) / (this.verticalScale[this.verticalScale.length - 1])
            }
        },
        data() {
            return {
        ...
        methods: {
            ...
            calcHorizontalX(index) {
                return this.scaleOffset - this.textOffset + (index * this.horizontalSpace)
            },
