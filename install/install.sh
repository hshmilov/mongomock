...
if [[ $* == *--run-system* ]]; then
    ...
    if [[ ${optional_parameters} != *"exclude"* ]]; then
        ...
        eval ./axonius.sh system up --exclude execution_correlator careful_execution_correlator nimbul qcore traiana_lab_machines stresstest stresstest_scanner json_file infinite_sleep diagnostics --all ${optional_parameters} --restart
    fi
    )
fi
