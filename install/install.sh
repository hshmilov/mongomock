...
if [[ $* == *--run-system* ]]; then
    ...
    if [[ ${optional_parameters} != *"exclude"* ]]; then
        eval ./axonius.sh system up --services general_info careful_execution_correlator --adapters ${optional_parameters} --restart
    else
        eval ./axonius.sh system up --exclude qcore traiana_lab_machines stresstest stresstest_scanner json_file infinite_sleep diagnostics --all ${optional_parameters} --restart
    fi
    )
fi
